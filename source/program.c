 ...
 * - June 19  2013: modified to print a list of CloogPrograms
 */
...
{ int i, j ;
  ...
  while (program) {
    ...
    if (program->options != NULL)
      fprintf(file,"+-- options = (present but not printed).\n");
    else
      fprintf(file,"+-- options = NULL.\n");
...
void cloog_program_sort_ascending(CloogProgram **program) {
  ...
  /* verify that all scops have coordinates in their extensions*/
  if(!cloog_program_verify_coordinates(*program))
    return;
  ...
  while (listend != head) {
    ...
    while (node->next != listend) {
      ...
      co = osl_generic_lookup(node->options->scop->extension,
                              OSL_URI_COORDINATES);
      nco = osl_generic_lookup(node->next->options->scop->extension,
                               OSL_URI_COORDINATES);
 ...
 * makes sure that all CloogPrograms contain a pointer to osl_scop_t in 
 * CloogOptions and all such SCoPs contain a Coordinates in their extension
 *
 * \param[in] program address of a pointer to a list of CloogPrograms
...
int cloog_program_verify_coordinates(CloogProgram * program) {
  ...
  CloogProgram *tmp = program;
  while (tmp) {
    scop = program->options->scop;
    if (scop==NULL) {
        cloog_msg(tmp->options, CLOOG_WARNING,
                  "Missing SCoP in program Options\n");
        return 0;
    }
    coordinates = osl_generic_lookup(scop->extension, OSL_URI_COORDINATES);
    //  - all scops contain coordinates
    if (coordinates == NULL) {
        cloog_msg(tmp->options, CLOOG_WARNING,
                  "Missing Coordinates in SCoP Extensions\n");
        return 0;
    }
    tmp = tmp->next;
  }
...
int cloog_program_osl_pprint(FILE * file, CloogProgram * program,
  ...
  while (program) {
    ...
    scop = program->options->scop;
    if (scop && !program->options->compilable && !program->options->callable) {
      coordinates = osl_generic_lookup(scop->extension, OSL_URI_COORDINATES);
      if (coordinates) {
        if (!orig_file_open) {
          original = fopen(coordinates->name, "r");
          if (!original) {
            cloog_msg(program->options, CLOOG_WARNING,
      ...
      else { // coordiantes not found
        cloog_msg(program->options, CLOOG_WARNING,
                  "unable to find coordinates in the SCoP\n");
        return 0;
      }
      ...
      root = cloog_clast_create(program, program->options);
      clast_pprint(file, root, coordinates->indent, program->options);
      ...
      if (program->next==NULL) { //last SCoP
        fclose(original);
        return 1;
      }
    }
...
CloogOptions * options ;
{
  ...
  while (program) {
    if (program->language == 'f')
      program->options->language = CLOOG_LANGUAGE_FORTRAN ;
    else
      program->options->language = CLOOG_LANGUAGE_C ;
 
#ifdef CLOOG_RUSAGE
    print_comment(file, program->options, "Generated from %s by %s in %.2fs.",
		          program->options->name, cloog_version(), program->options->time);
#else
    print_comment(file, program->options, "Generated from %s by %s.",
	                program->options->name, cloog_version());
#endif
#ifdef CLOOG_MEMORY
    print_comment(file, program->options, "CLooG asked for %d KBytes.",
                  program->options->memory);
    cloog_msg(CLOOG_INFO, "%.2fs and %dKB used for code generation.\n",
	            program->options->time,program->options->memory);
#endif
     ...
     */
    if (program->options->compilable && (program->language == 'c'))
    { /* The headers. */
      if(!print_headers){
      ...
      if (!print_main)
        ...
        print_iterator_declarations(file, program, program->options);
      ...
      indentation += 2 ;
    } else if (program->options->callable && program->language == 'c') {
      print_callable_preamble(file, program, program->options);
      indentation += 2;
    }
    
    root = cloog_clast_create(program, program->options);
    clast_pprint(file, root, indentation, program->options);
    cloog_clast_free(root);
...
{ 
  while(program){
    ...
    cloog_options_free(program->options) ;
...
CloogProgram *cloog_program_read(FILE *file, CloogOptions *options)
{
  ...
  while (input) {
    *p_ptr = cloog_program_alloc(input->context, input->ud,
                                 input->options);
    (*p_ptr)->options = input->options;
...
CloogOptions * options ;
{
  ...
  while (program) {
    ...
    program->options->memory = 0 ;
#endif
    
  
    if (program->options->override)
    {
      cloog_msg(program->options, CLOOG_WARNING,
    ...
    { /* Playing with options may be dangerous, here are two possible issues :
       ...
       */
      if ((program->nb_scattdims > program->options->l) 
          && (program->options->l >= 0))
      {
        cloog_msg(program->options, CLOOG_WARNING,
        ...
        "to override).\n") ;
        program->options->l = program->nb_scattdims ;
      }
       ...
       */
      if (((program->options->f > 1) || (program->options->f < 0)) &&
          ((program->options->l > program->nb_scattdims) || 
           (program->options->l < 0)))
      {
        cloog_msg(program->options, CLOOG_WARNING,
    ...
    { loop = program->loop ;
      ...
      fscanf(status,"%d",&(program->options->memory)) ;
      fclose(status) ;
#endif
      
      if ((!program->options->nosimplify) && (program->loop != NULL))
        loop = cloog_loop_simplify(loop, program->context, 0,
                                   program->nb_scattdims, program->options);
