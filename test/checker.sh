...
## Name of the group of files to test
readonly TEST_NAME="${1}"

## List of test file prefixes and individual options spaces between the
## elements of one test are represented with '%', e.g., "file -f -1"
## is "file%-f%-1".
readonly TEST_FILES="${2}"

## Options for all the tests in the group
readonly TEST_GLOBAL_OPTIONS="${3}"

## Extension of the input file
readonly TEST_INPUT_EXTENSION="${4}"

## Extension of the generated file
readonly TEST_OUTPUT_EXTENSION="${5}"

## - "generate" to simply test code generation (default)
## - "regenerate" to replace the original file with the generated one in the
##   case of a check fail
##   !!! USE WITH CARE !!!
## - "valgrind" to test the valgrind output on code generation
## - "hybrid" compare source to source and if this fails run them afterwards
readonly TEST_TYPE="${6}"
...
log_test ()
{
  dest="${1}" color="${2}" state="${3}" test_name="${4}" test_time="${5}"
  details="${6}"
...
log_test_set ()
{
  log_file="${1}" summary_file="${2}" color="${3}" state="${4}"
  test_set_name="${5}"
    ...
    if [ "${errors}" = "1" ]; then
      ...
      if [ "${leaks}" = "${reachable}" ]; then
        ...
        test_passed "${input}" "${elapsed_time}" "${options}" \
                    ...
                    "${TEXT_DIM}(See ${TEXT_UL}${input_log}" \
                    "${TEXT_NORMAL}${TEXT_DIM}.)"
      else
